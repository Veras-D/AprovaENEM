...
- **FR-10b (Tiered AI Tutor Access & Daily Rate Limiting)**: Practice questions, quizzes, scoring, and static step-by-step resolutions are 100% free and unlimited for all students. Socratic AI Tutor consultations (`POST /questions/{id}/ask`) are limited to **1 free consultation per day** for guest (`ROLE_ANONYMOUS_STUDENT`) and free registered (`ROLE_STUDENT`) accounts (resetting daily at 00:00 BRT). Subscribed students (`ROLE_PREMIUM_STUDENT`) receive **unlimited** Socratic AI Tutor consultations.
