...
gantt
    ...
    Password Storage Pepper Hardening (S3-07b)           :active, s3_5b, after s3_5, 1d
    Back-end CI Quality Gate Verification (S3-10)        :s3_7, after s3_5b, 1d
    Backend Security and Test Integrity Audit (S3-11)    :s3_8, after s3_7, 1d
    JAM 1 Final Delivery and Audit (S3-12)               :s3_9, after s3_8, 1d

    section JAM 2 - Front-end (Sprint 4)
    Historical Question Bank Ingestion (S4-00)           :s4_0, 2026-10-01, 2d
    React 18 TypeScript and Tailwind Setup (S4-01, S4-02):s4_1, after s4_0, 3d
...
| **JAM 1: Month 2**   | **Sprint 3** (Sept 20–22, 2026) | Verification, Hardening, Contracts & JAM 1 Delivery    |      14       |        9        | **IN PROGRESS (64%) 🚀** | Domain Unit Tests (`S3-01` ✅), App Unit Tests (`S3-02` ✅), MockMvc Security (`S3-03` ✅), Testcontainers IT (`S3-04` ✅), JaCoCo 80% (`S3-05` ✅), Redis benchmark (`S3-06` ✅), k6 Load & Stress (`S3-06b` ⏳), Swagger UI (`S3-08` ✅), Newman Contract Suite (`S3-09` ✅), Chaos fallback (`S3-07` ✅), Password Pepper (`S3-07b` ⏳), CI Gate with Pre-Flight Smoke (`S3-10` ⏳), Security & Test Integrity Audit (`S3-11` ⏳), JAM 1 Submission & Media Kit (`S3-12` ⏳) |
| **JAM 2: Month 3**   | **Sprint 4** (Oct 01–10, 2026)  | Frontend Setup, Design Tokens, State & Accessibility   |       9       |        0        |       `PLANNED 📅`       | Question bank bulk ingestion (`S4-00`), React 18 + TS + Vite, Tailwind dark mode, Zustand store, TanStack Query, KaTeX math formulas, Mobile shell, Accessibility baseline (`S4-07`), Terms modal (`S4-08`)                                                                                                                                                 |
...
| **JAM 2: Month 3**   | **Sprint 6** (Oct 21–31, 2026)  | Full-Stack Integration, E2E Testing & Cloud Deployment |       9       |        0        |       `PLANNED 📅`       | Vitest RTL suite, Cypress E2E, Playwright cross-browser, Production Nginx container, Cloud HTTPS deployment, Security & Test Integrity audit (`S6-06`), Partner presentation (`S6-07`), Visual media kit (`S6-08`), LGPD portal (`S6-09`)                                                                                                                   |
...
| **S3-06b** | **Backend Load & Stress Testing with Grafana k6** | `P1` | 5 pts | `PENDING ⏳` | Containerized Grafana k6 load & stress testing suite (`tests/stress/`) simulating: (1) Exam Rush scenario (1,000+ VUs querying questions and submitting answers against Redis L2 cache and PostgreSQL), (2) Socratic AI Consultation Burst (500 concurrent threads testing Token Bucket 429 throttling, atomic quota race safety, and Resilience4j circuit breaker stability under simulated Gemini latency), and (3) Gamification Event Ingestion (event flooding testing RabbitMQ backpressure and consumer lag). Produces automated latency percentiles (P50, P90, P95, P99) and saturation report (`docs/benchmarks/jam1-k6-stress-benchmarking.md`). |
...
| **S3-07b** | **Password Storage Pepper Hardening (HMAC-SHA256 + BCrypt)** | `P1` | 3 pts | `PENDING ⏳` | Implement application-level secret pepper layered over BCrypt in `auth-service` via a configurable `PasswordEncoderPort` adapter/decorator. Computes `HMAC-SHA256(rawPassword, pepper)` prior to BCrypt hashing ($2^{10}$ rounds), externalizing the secret pepper to an environment variable (`AUTH_PASSWORD_PEPPER`) with dual-check backward compatibility for legacy hashes, completely defeating offline dictionary and brute-force cracking even if the PostgreSQL database is breached in isolation. |
...
| **S3-10** | **Back-end GitHub Actions CI Quality Gate & Pre-Flight Smoke Suite** | `P0` | 5 pts | `PENDING ⏳` | 7-stage CI verification for backend (`javac -Werror`, Checkstyle, PMD, PMD CPD, Trivy, Gitleaks, JaCoCo, Pre-Flight Smoke Suite < 15s verifying actuator health and golden path, Newman) 100% green on `main`. |
| **S3-11** | **Multi-Stage Backend Security & Test Suite Integrity Audit** | `P0` | 5 pts | `PENDING ⏳` | Comprehensive 5-stage security and test legitimacy audit: (1) AI-driven threat modeling & code review (OWASP API Top 10), (2) Automated DAST & dependency audit (Trivy, Gitleaks, ZAP), (3) Live interactive runtime penetration testing (JWT signature tampering, CORS bypass fuzzing, perimeter isolation check, header spoofing rejection, Token Bucket flooding, SQLi/pgvector injection), (4) Test Suite Quality & Legitimacy Audit: static & dynamic audit of all 336+ tests to eliminate test smells, vacuous/tautological assertions (e.g. `assertTrue(true)`), over-mocking masking real execution paths, swallowed exceptions, and assertion-free methods; verified via mutation testing / mutant fault injection ensuring regressions in TRI scoring, streak freezes, daily goals, and security filters trigger test failures, (5) Formal Audit Report (`docs/audit/jam1-backend-security-audit.md`) certifying zero vulnerabilities and 100% test integrity. |
