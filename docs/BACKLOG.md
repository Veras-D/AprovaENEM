...
gantt
    ...
    Historical Question Bank Ingestion (S3-13)           :done, s3_13, after s3_9, 2d
    ...
    Supply Chain & Perimeter Hardening (S3-16)           :done, s3_16, after s3_15, 1d
...
## 4. Sprint 3: Back-end Hardening, Verification & JAM 1 Delivery (JAM 1 — COMPLETED ✅)
...
| **S3-13** | **Historical Question Catalog Bulk Ingestion & Reconciliation (2019–2023)** | `P1` | 5 pts | **DONE ✅** | *(Brought forward from JAM 2 Sprint 4)* Ingestion pipeline engine updated and executed for representative ENEM editions (2019–2023): ingested, verified, and reconciled 25 authentic questions across 5 editions covering all 4 knowledge areas (Matemática, Ciências da Natureza, Ciências Humanas, Linguagens) and 10 disciplines; hydrated with official INEP microdados TRI psychometric parameters (discrimination $a$, difficulty $b$, guessing $c$) and ENEM competency tags ($H_1$–$H_{30}$); generated and mounted lossless 300 DPI WebP visual diagram assets to `exam-assets-data` volume, served via Nginx `/assets/questions/` with immutable 1-year cache headers and security headers; reproducible Flyway migration `V7__historical_question_catalog_seed.sql` applied to `exam_db`; defensive LaTeX regex lookbehinds preventing Brazilian currency `R\$` false-positives; verified via 11 pytest unit/batch tests, `ExamPersistenceIT` Testcontainers integration tests, 7/7 smoke probes, and 42/42 Newman contract tests (75/75 assertions). |
...
| **S3-16** | **Supply Chain, Ingress Perimeter & Resource Boundary Hardening** | `P1` | 3 pts | **DONE ✅** | Implemented supply chain, edge proxy, and resource limit hardening: (1) Supply Chain BOM Alignment: Upgraded root Spring Boot parent to `3.3.11` and pinned `<tomcat.version>10.1.35</tomcat.version>` and `<netty.version>4.1.118.Final</netty.version>` to patch embedded runtime CVEs (Tomcat RCE `CVE-2025-24813`, Netty SNI bypass `CVE-2026-75595`, Spring Security 72-byte truncation `CVE-2025-22228`); (2) Ingress Reverse Proxy Hardening: Upgraded Edge Nginx to `nginx:1.27-alpine`, added `server_tokens off;` to suppress version leakage, and explicitly restored security headers (`X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `X-XSS-Protection`, `Referrer-Policy`) in `location /assets/questions/` to prevent child block inheritance loss; (3) Resource Boundary Enforcement: Clamped maximum pagination size (`size <= 50`) on `QuestionFilterCommand` in `exam-service` to prevent heap exhaustion / OOM denial-of-service, hardened `backend/ingestion-service/Dockerfile` by creating and executing as unprivileged user `appuser:appuser` (`uid=1000`), and declared explicit CPU and memory resource limits (`deploy.resources.limits`) across all 14 containers in `docker-compose.yml`. Verified with 100% green tests, 0 Checkstyle violations, 0 PMD violations, 7/7 pre-flight smoke probes, and 42/42 Newman contract requests (75/75 assertions). |
