...
gantt
    ...
    LGPD Portability & Erasure Backend APIs (S3-14)      :s3_14, after s3_13, 1d
    Security Audit Critical P0 Hotfixes (S3-15)          :s3_15, after s3_14, 1d
...
| **S3-14** | **LGPD Art. 18 Data Portability & Irrevocable Account Erasure Backend APIs** | `P0` | 5 pts | `PENDING ⏳` | *(Brought forward from JAM 2 Sprint 6)* Implementation of core backend LGPD compliance endpoints in `auth-service`: Art. 18 self-service data portability JSON export (`GET /api/v1/auth/export`) aggregating user profile, practice session attempts, and gamification activity; and 1-click irrevocable account erasure/anonymization (`DELETE /api/v1/auth/me`) cascading data scrubbing across `auth-service`, `exam-service`, and `notification-service` with transactional consistency; verified via automated contract tests in Newman and live Swagger UI (`/swagger-ui.html`) synchronization. |
| **S3-15** | **Backend Security Audit Critical P0 Remediation & Hardening** | `P0` | 5 pts | `PENDING ⏳` | Remediate critical security, data integrity, and performance defects identified in Phase 1–5 audit: (1) Practice Session BOLA (`exam-service`): Enforce ownership checks against calling principal (`userId` or `anonymousSessionId`) on `GET /api/v1/sessions/{id}`, `/attempts`, `/complete`, and `/diagnostic`, preventing unauthorized cross-student tampering; (2) Question Catalog BFLA (`exam-service`): Secure `PATCH /api/v1/questions/{id}/status` with `@PreAuthorize("hasRole('ADMIN')")`; (3) Docker Compose Environment Alignment: Inject `JWT_SECRET=${JWT_SECRET}` into `exam-service` in `deployments/docker-compose.yml`, preventing signature validation mismatches; (4) Double-BCrypt Overhead & Fail-Fast Pepper (`auth-service`): Refactor `AuthService.login()` and `BCryptPasswordEncoderAdapter` to single-pass verification, cutting login CPU latency by half and eliminating side-channel timing discrepancies; add fail-fast validation for `AUTH_PASSWORD_PEPPER` at startup in non-test profiles; (5) Test Suite Hygiene & Security Filter Re-enabling: Add concrete assertions to `TutorChatRepositoryAdapterTest:177` and `RabbitMQNotificationPublisherAdapterTest:54`, and re-enable Spring Security filters in WebMvc slice tests (removing `addFilters = false`) with `@WithMockUser`. |
